...
- [x] Skip raid/spotlight schedule tree-building — deferred alongside text-only raid-boss art at the time of this phase. A text-only "event extras" bonuses block (Community Day/raid-hour/spotlight/season bonus text, no icons) was ported for the expanded card, since that data isn't boss/sprite art.
- [x] **Later reversed for this view**: static Pokémon sprites, the spotlight bonus icon, Dynamax/Shadow/Gigantamax sprite effects, Super Mega shield badges, Hundo CP badges, and the full day-by-day raid schedule breakdown were built for Timeline cards (see "Timeline Pokemon Sprites & Raid Schedule" plan and `pogo-cal/docs/migration/OPEN-DECISIONS.md`'s updated raid-boss-art row) — animated sprites remain out of scope

...
- [ ] Rebuild components/pages in the order `SCREEN-AND-FEATURE-MAP.md` specifies — chunked into 5 groups (see [progress notes](progress/phase-5-gather-page.md) for the chunking and group 1's completion): 1) gather page core ✅, 2) side menu/filters, 3) settings page, 4) search strings page, 5) preset queries (highest complexity, last)
